...
<script>>
// ==================== CHART INTEGRATION FOR AI SUMMARIES ====================
// These modifications integrate AI summaries with your existing chart functions
// Add these after your chart creation functions or replace the existing ones

/**
 * MODIFIED: Create Retention Chart with AI Summary
 * Replace or modify your existing createRetentionChart function
 */
function createRetentionChart(data) {
    const canvas = document.getElementById('retentionChart');
    if (!canvas) {
        console.error('[CHART] Retention chart canvas not found');
        return;
    }
    ...
    if (!data || data.length === 0) {
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#9ca3af;">No retention data available</p>';
        return;
    }

    try {
        // Create Chart.js instance
        window.retentionChartInstance = new Chart(canvas, {
            type: 'line',
            data: {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: ctx => 'Retention: ' + ctx.parsed.y.toFixed(1) + '%'
                        }
                    }
                }
            }
        ...
        console.log('[CHART] Retention chart created successfully');

        // Generate AI Summary
        AIChartSummary.generateSummaryForChart(
            window.retentionChartInstance,
            'retention',
            'retention-ai-summary'
        );

    } catch (error) {
        console.error('[CHART] Error creating retention chart:', error);
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#ef4444;">Error loading chart</p>';
    }
}

/**
 * MODIFIED: Create Behavior Chart with AI Summary
 * Replace or modify your existing createBehaviorChart function
 */
function createBehaviorChart(data) {
    const canvas = document.getElementById('behaviorChart');
    if (!canvas) {
        console.error('[CHART] Behavior chart canvas not found');
        return;
    }
    ...
    if (!data || data.length === 0) {
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#9ca3af;">No behavior data available</p>';
        return;
    }
    ...
    try {
        ...
        console.log('[CHART] Behavior chart created successfully');

        // Generate AI Summary
        AIChartSummary.generateSummaryForChart(
            window.behaviorChartInstance,
            'behavior',
            'behavior-ai-summary'
        );

    } catch (error) {
        console.error('[CHART] Error creating behavior chart:', error);
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#ef4444;">Error loading chart</p>';
    }
}

/**
 * MODIFIED: Create Revenue Chart with AI Summary
 * Replace or modify your existing createRevenueChart function
 */
function createRevenueChart(data) {
    const canvas = document.getElementById('revenueChart');
    if (!canvas) {
        console.error('[CHART] Revenue chart canvas not found');
        return;
    }
    ...
    if (!data || data.length === 0) {
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#9ca3af;">No revenue data available</p>';
        return;
    }
    ...
    try {
        ...
        console.log('[CHART] Revenue chart created successfully');
        ...
        console.error('[CHART] Error creating revenue chart:', error);
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#ef4444;">Error loading chart</p>';
    }
}

/**
 * MODIFIED: Create Trends Chart with AI Summary
 * Replace or modify your existing createTrendsChart function
 */
function createTrendsChart(data) {
    const canvas = document.getElementById('trendsChart');
    if (!canvas) {
        console.error('[CHART] Trends chart canvas not found');
        return;
    }
    ...
    if (!data || data.length === 0) {
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#9ca3af;">No trends data available</p>';
        return;
    }
    ...
    try {
        ...
        console.log('[CHART] Trends chart created successfully');

        // Generate AI Summary
        AIChartSummary.generateSummaryForChart(
            window.trendsChartInstance,
            'trends',
            'trends-ai-summary'
        );

    } catch (error) {
        console.error('[CHART] Error creating trends chart:', error);
        canvas.parentElement.innerHTML = '<p style="text-align:center;padding:40px;color:#ef4444;">Error loading chart</p>';
    }
}

/**
 * MODIFIED: Tab Switch Function with AI Summary Support
 * Replace or modify your existing switchTab function
 */
function switchTab(tabName) {
    ...
    if (activeBtn) {
        ...
        activeBtn.style.color = '#5E72E4';
        activeBtn.style.borderBottom = '3px solid #5E72E4';
...
function loadTabData(tabName) {
    console.log('[TAB] Loading data for:', tabName);

    // Add your existing data loading logic here
    // For example:
    switch(tabName) {
        case 'retention':
            // Load retention data and create chart
            // Example: loadRetentionData();
            break;
        case 'behavior':
            // Load behavior data and create chart
            // Example: loadBehaviorData();
            break;
        case 'revenue':
            // Load revenue data and create chart
            // Example: loadRevenueData();
            break;
        case 'trends':
            // Load trends data and create chart
            // Example: loadTrendsData();
            break;
    }
}

/**
 * Refresh all reports and regenerate AI summaries
 */
function refreshReports() {
    console.log('[REFRESH] Refreshing all reports...');
    
    // Cancel any ongoing AI summary requests
    AIChartSummary.cancelAllRequests();
    
    // Show notification
    showNotification('Refreshing data and AI insights...', 'info');
    
    // Reload current tab data
    const activeTab = document.querySelector('.tab-btn.active');
    if (activeTab) {
        const tabName = activeTab.textContent.toLowerCase().replace(/\s+/g, '_');
        loadTabData(tabName);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('[INIT] Initializing chart reports with AI summaries...');
    
    // Update last updated time
    updateLastUpdatedTime();
    
    // Load initial tab data
    loadTabData('retention');
    
    console.log('[INIT] Initialization complete');
});

console.log('[INTEGRATION] Chart integration with AI summaries loaded successfully');
</script>
